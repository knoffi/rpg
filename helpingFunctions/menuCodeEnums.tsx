...
export const adjustOfferPrice=(offer:Offer,fits:association[],misfits:association[], basePrice:BasePrice)=>{
  ...
  offer.product.associations.forEach(productFit=>{
    if(!passedIncomeFit){
      if(productFit === association.rich){
        basePriceFactor=basePrice.rich * 1.0 / standardBasePrice.rich
        passedIncomeFit=true;
      } else {
        if(productFit === association.sophisticated){
          basePriceFactor=basePrice.wealthy * 1.0 / standardBasePrice.wealthy
          passedIncomeFit=true;
        } else {
          if(productFit === association.worker){
            basePriceFactor=basePrice.modest * 1.0 / standardBasePrice.modest
            passedIncomeFit=true;
          } else {
            if(productFit === association.poor){
              basePriceFactor=basePrice.poor * 1.0 / standardBasePrice.poor
              passedIncomeFit=true;
            }
          }
        }
  ...
  if(!passedIncomeFit){
    basePriceFactor=(basePrice.modest+basePrice.wealthy) * 1.0 / (standardBasePrice.modest + standardBasePrice.wealthy)
  }
  const newPrice = Math.floor(offer.product.copperPrice * (basePriceFactor!) * (1.0+ tavernPriceCount / 20.0));
