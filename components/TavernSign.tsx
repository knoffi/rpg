...
const SIGN_WIDTH= 357;
const SIGN_HEIGHT = 160;
const TRYOUT_FONTS =false;
const TRYOUT_COLORS =false;

const fontFamilies = TRYOUT_FONTS ? ["EnglishGothic", "Breitkopf", "primitive", "strassburg", 'chomsky', 'canterbury', 'deutschGothic', 'deutschZier', 'fiddum', 'metalMacabre', 'pau', 'rapscallion'] : ["primitive"] ;
const fontSizes = TRYOUT_FONTS? [32, 44, 32, 68, 34, 38, 36, 34, 38, 26, 36, 46] : [34];
const fontColors = TRYOUT_COLORS ? ["gold", "goldenrod", "navajowhite", "firebrick", "silver", "gainsboro"] : ["goldenrod"] ;

const TopFontControllButtons=(props:{setFamilyIndex:(index:number)=>void,familyIndex:number,fontFamilies:string[]})=>{
  return <View style={{ flexDirection: "row", justifyContent: "space-evenly" }}>
  <Button title={"Family Forward"} onPress={() => { props.setFamilyIndex((props.familyIndex + 1) % props.fontFamilies.length); }}>
  </Button>
  <Button title={"Family Back"} onPress={() => { props.setFamilyIndex((props.fontFamilies.length + props.familyIndex - 1) % props.fontFamilies.length); }}>
  </Button>
</View>
}

const BottomColorControlButtons=(props:{setColorIndex:(index:number)=>void,colorIndex:number,fontColors:string[]})=>{
  return <View style={{ flexDirection: "row", justifyContent: "space-evenly" }}>
      <Button title={"Color Forward"} onPress={() => { props.setColorIndex((props.colorIndex + 1) % props.fontColors.length); }}>
      </Button>
      <Button title={"Color Back"} onPress={() => { props.setColorIndex((props.fontColors.length + props.colorIndex - 1) % props.fontColors.length); }}><Image source={require("../assets/splash.png")} style={{}}></Image>
      </Button>
    </View>
}
...
export const TavernSign = (props: { nameText: string}) => {
  ...
  return <View>{topComponents}
    <ImageBackground source={require("../assets/tavernSign.png")} style={{ width: SIGN_WIDTH, height: SIGN_HEIGHT, alignSelf: "center", flexDirection: "column", justifyContent: "center" }}>
      ...
      </View></ImageBackground>{bottomComponents}
  </View>;
};
