...
const PriceDescriptionDialog = (props: {
    descriptionDialogData: descriptionDialogData;
    onDismiss: () => void;
}) => {
    return (
        <Portal>
            <Dialog
                visible={props.descriptionDialogData.open}
                onDismiss={props.onDismiss}
            >
                <Dialog.Content>
                    <Text>
                        <Text style={{ fontSize: 18 }}>
                            {props.descriptionDialogData.price.toString() +
                                ' ' +
                                props.descriptionDialogData.currencyName +
                                ' stands for the average price which ' +
                                props.descriptionDialogData.income +
...
export const QuestScene = (props: {
    ...
    const [descriptionDialogData, setDialogData] = useState({
        open: false,
    ...
    const [priceSetterData, setPriceSetterData] = useState({
        open: false,
        income: association.poor,
        priceText: props.basePrice.poor.toString(),
        price: 12,
    });
    const [currencySetterData, setCurrencySetterData] = useState({
        open: false,
    ...
    const setPriceText = (text: string) => {
        setPriceSetterData({
            open: priceSetterData.open,
            income: priceSetterData.income,
            priceText: text,
            price: priceSetterData.price,
        });
    };
    ...
    const onDialogDismiss = () => {
        setDialogData({
            open: false,
            income: descriptionDialogData.income,
            price: descriptionDialogData.price,
            currencyName: descriptionDialogData.currencyName,
            jobExamples: descriptionDialogData.jobExamples,
        });
        setPriceSetterData({
            open: false,
            income: priceSetterData.income,
            price: priceSetterData.price,
            priceText: priceSetterData.priceText,
        });
        setCurrencySetterData({
            open: false,
            currency: props.basePrice.currency,
    ...
    return (
        <ScrollView>
            ...
            <PriceSetDialog
                ...
                income={priceSetterData.income}
                priceText={priceSetterData.priceText}
                lastPrice={priceSetterData.price}
                setPrice={onPriceSet}
                setPriceText={setPriceText}
            ></PriceSetDialog>
            <CurrencySetDialog
                open={currencySetterData.open}
                currency={currencySetterData.currency}
