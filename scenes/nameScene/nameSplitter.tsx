export const nameSplitter = (drinkName: string, characterMax: number) => {
    ...
    while (splitIndexFound) {
        splitIndex =
            splitIndex +
            1 +
            drinkName.substring(splitIndex + 1, characterMax + 1).indexOf(' ');
        if (!splitIndex) {
            splitIndex =
                splitIndex +
                1 +
                drinkName
                    .substring(splitIndex + 2, drinkName.length)
                    .indexOf(' ');
            if (!splitIndex) {
                console.log(
                    'drink/food name has no empty space or while loop has mistake'
                );
                return drinkName;
            } else {
        ...
        if (
            !drinkName.substring(splitIndex + 1, characterMax + 1).includes(' ')
        ) {
            splitIndexFound = false;
        }
    }
